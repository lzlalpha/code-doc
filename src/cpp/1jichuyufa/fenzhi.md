...
以上代码运行后输出的结果为 `daimalong` 和 `Hello World`，如果不想让下面分支的语句被运行就需要 break 了，具体例子可看下面的示例。
...
以上代码运行后输出的结果为 daimalong，因为 break 的存在，接下来的语句就不会继续被执行了。最后一个语句不需要 break，因为下面没有语句了。
